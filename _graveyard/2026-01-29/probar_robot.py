    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if response.status_code == 200:
                self.stdout.write(
                    self.style.SUCCESS("¡Conexión Exitosa! (Saltando SSL)")
                )
                ...
                if "794" in texto_pag or "Medida" in texto_pag:
                    self.stdout.write(
                        self.style.SUCCESS(
                            "CONFIRMADO: Vemos información de la medida en el texto."
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            "OJO: Entramos, pero no veo el número 794. Tal vez la estructura cambió."
                        )
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Error: SUTRA respondió código {response.status_code}"
                    )
                )
