...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--yes", action="store_true", help="Run without confirmation"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not options.get("yes"):
            confirm = (
                input(f"About to execute SQL at {sql_path}. Continue? [y/N]: ")
                .strip()
                .lower()
            )
            if confirm != "y":
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(sql)
            self.stdout.write(
                self.style.SUCCESS("HNSW index SQL executed successfully")
            )
