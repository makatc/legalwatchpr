...
class Article(models.Model):
    ...
    content_hash = models.CharField(
        max_length=32, blank=True, null=True, help_text="Hash MD5 del contenido"
    )
...
class BillVersion(models.Model):
    ...
    def __str__(self):
        return f"{self.bill.number} - {self.version_name}"
...
class SystemSettings(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, help_text="Activar/desactivar el servicio globalmente"
    )
    active_days = models.CharField(
        max_length=50, default="0,1,2,3,4", help_text="Días activos: 0=Lunes, 6=Domingo"
    )
    high_freq_start = models.TimeField(
        default=time(8, 0), help_text="Inicio del horario intensivo"
    )
    high_freq_end = models.TimeField(
        default=time(17, 0), help_text="Fin del horario intensivo"
    )
    high_freq_interval = models.IntegerField(
        default=15, help_text="Intervalo en modo intensivo (minutos)"
    )
    low_freq_interval = models.IntegerField(
        default=120, help_text="Intervalo en modo pasivo (minutos)"
    )
...
class UserProfile(models.Model):
    ...
    webhook_measures = models.URLField(
        max_length=500, blank=True, null=True, verbose_name="Webhook Medidas"
    )
    webhook_commissions = models.URLField(
        max_length=500, blank=True, null=True, verbose_name="Webhook Comisiones"
    )
    webhook_keywords = models.URLField(
        max_length=500, blank=True, null=True, verbose_name="Webhook Keywords"
    )
    webhook_general = models.URLField(
        max_length=500, blank=True, null=True, verbose_name="Webhook General"
    )
