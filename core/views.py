...
try:
    ...
    def search_keyword_only(*args, **kwargs):
        return []

    def search_semantic_only(*args, **kwargs):
        return []


# CORRECCIÓN AQUÍ: Importar desde .helpers en lugar de .utils
from .helpers import fetch_latest_news, generate_ai_summary
from .models import (
    Article,
    Bill,
    Event,
...
AVAILABLE_COMMISSIONS = [
    ...
    "Autonomía Municipal",
    "Bienestar Social",
    "Calendario y Reglas Especiales",
    "Cooperativismo",
    "Desarrollo Económico",
    "Desarrollo Integrado de la Región Sur",
    "Desarrollo Integrado de la Región Oeste",
    "Desarrollo Integrado de la Región Norte",
    "Desarrollo Integrado de la Región Este",
    "Desarrollo Integrado de la Región Centro",
    "Educación, Arte y Cultura",
    "Energía",
    "Ética",
    "Hacienda y Presupuesto",
    "Impacto Comunitario",
    "Innovación, Telecomunicaciones",
    "Jurídico",
    "Juventud",
    "Nombramientos",
    "Pequeños y Medianos Negocios",
    "Preparación, Reconstrucción y Reorganización",
    "Probidad y Ética Gubernamental",
    "Proyectos Estratégicos y Energía",
...
def noticias(request):
    ...
    if query:
        if search_method == "hybrid":
            search_results = search_documents(query, limit=50)
        elif search_method == "semantic":
            search_results = search_semantic_only(query, limit=50)
        elif search_method == "keyword":
            search_results = search_keyword_only(query, limit=50)
        else:
            search_results = []
        ...
        if search_results:
            ...
            articles = Article.objects.filter(id__in=article_ids).order_by("-published_at")
        else:
            articles = Article.objects.none()
    
    # Calculate stats
    today = datetime.now().date()
    today_count = Article.objects.filter(published_at__date=today).count()
    ...
    if query:
        try:
            ...
            search_stats = {
                ...
                'articles_with_search_vector': 0,
                'search_vector_coverage': 0,
            }
    ...
    context = {
        "page_obj": page_obj,
        "articles": articles,  # For compatibility
        "query": query,
...
def medidas(request):
    ...
    bills = Bill.objects.all().order_by(
        "-last_updated" if hasattr(Bill, "last_updated") else "-id"
    )
    if query:
        bills = bills.filter(
            models.Q(title__icontains=query) | models.Q(number__icontains=query)
        )
...
def dashboard_configuracion(request):
    ...
    if request.method == "POST":
        if "add_measure" in request.POST:
            ...
            if m_id:
                MonitoredMeasure.objects.get_or_create(sutra_id=m_id, is_active=True)
        elif "add_commission" in request.POST:
            c_name = request.POST.get("commission_name", "").strip()
            if c_name:
                MonitoredCommission.objects.get_or_create(name=c_name, is_active=True)
        elif "add_keyword" in request.POST:
            term = request.POST.get("keyword", "").strip()
            if term:
                Keyword.objects.get_or_create(term=term)
