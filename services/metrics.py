...
class SearchMetrics:
    ...
    def ndcg_at_k(retrieved: List[int], relevant: Dict[int, float], k: int) -> float:
        ...
        for rank, doc_id in enumerate(retrieved[:k], start=1):
            ...
            dcg += rel / (
                1.0 if rank == 1 else (rank**0.5)
            )  # log2(rank+1) simplificado
        ...
        idcg = sum(
            rel / (1.0 if rank == 1 else (rank**0.5))
...
def evaluate_search_quality(
    ...
    for query_data in test_queries:
        ...
        results, latency = latency_tracker.measure(
            search_func, query, limit=max(k_values) if k_values else 20
        )
        ...
        precision_for_f1 = SearchMetrics.precision_at_k(
...
def format_evaluation_report(evaluation: Dict[str, Any]) -> str:
    ...
    if mean_lat < 200:
        ...
        report.append(
            f"❌ Latencia media = {mean_lat:.1f} ms (objetivo: <200 ms, LENTA)"
        )
